...
class BQD:
    ...
    def __init__(self):
        # self.pageIndex = 1
        self.user_agent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:63.0) Gecko/20100101 Firefox/63.0'
        #初始化headers
    ...
    #获得章节名和url
    def getPage(self,bookurl):
        try:
            url = 'https://www.biqudu.com'+bookurl
            #构建请求的request
            request = urllib.request.Request(url,headers = self.headers)
            #利用urlopen获取页面代码
            response = urllib.request.urlopen(request)
    ...
    def getPageItems(self,chapterurl):
        url = 'https://www.biqudu.com'+chapterurl
         #构建请求的request
        request = urllib.request.Request(url,headers = self.headers)
        #利用urlopen获取页面代码
        response = urllib.request.urlopen(request)
    ...
    def start(self):
        ...
        bookname1 = input()
        bookname=parse.quote(bookname1)#bookname1.encode("utf-8")
        # print(bookname)
        books=self.getBook(bookname)
        print(books)
        for book in books:
            # getPageItems(bookurl)
            chapters=self.getPage(book[1])
            print(chapters)
